...
class qemu_monitor:
    ...
    def write_phys_mem_file_to_disk():
        print('Connecting to Qemu monitor at %s:%d' % (qemu_monitor.HOSTNAME, qemu_monitor.PORT))
...
class kernel:
    ...
    def parse_and_eval(cmd):
        ...
        gdb.execute("set logging enabled") # older versions of gdb do not support set logging enabled on so we need to use this.
    ...
    read_byte = lambda addr: int(str(gdb.execute(f'''x/1bx {addr}''', to_string=True)).split()[-1], base=16)
    ...
    class task:
        ...
        def get_name():
            ...
            process_name = str(kernel.parse_and_eval('((struct task_struct*)%s)->comm' % task_struct))
...
class FuzzBkpt(gdb.Breakpoint):
    ...
    def __init__(self, target_dir, addr, program_name, checkname=True, bp_hits_required=1, target_base=0x555555554000, sym_path=None):
    ...
    def stop(self):
        ...
        if self.bp_hits < self.bp_hits_required:
            print('Hit bp %d time, but need to hit it %d times' % (self.bp_hits, self.bp_hits_required))
            return False
    ...
    def get_text_offset(self, target_file):
        output = subprocess.check_output(["readelf", "-S", target_file]).decode().split('\n')
    ...
    def save_orig_bytes(self, start_addr, num_bytes):
        if self.orig_bytes is None:
            ...
            print('Saving %d bytes from 0x%x to 0x%x' % (num_bytes, start_addr, self.start_orig_rip))
            self.start_orig_rip = start_addr
            addr_to_read = start_addr
            prepend_bytes = []
            for i in range(num_bytes):
